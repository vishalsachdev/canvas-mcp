...
- **#318 — "[Feature]: use confirmation token for delete tools"**, opened by **`zqian`** (external
  ...
  **Two more corrections, caught by the executor pass on this PR and independently re-verified:**

  (B) There's already a reusable pattern to build this on — but it's **one shared class**, not
  three separate implementations as this brief first framed it. `ConfirmationGuard`
  (`src/canvas_mcp/core/write_confirmation.py:38`) is instantiated once in
  `content_migrations.py` (`_CONTENT_MIGRATION_GUARD`) and four times in `messaging.py`
  (`_BULK_MESSAGE_GUARD`, `_SEND_CONVERSATION_GUARD`, `_REMINDER_GUARD`, `_CAMPAIGN_GUARD`).
  Only `student_write.py`'s `_check_token` is a standalone function, not an instance of the
  shared class. This makes #318 *more* mechanical than "apply an established pattern" implied:
  it's import-and-instantiate, not five independent implementations to harmonize.

  (C) `delete_assignment_with_confirmation` is not a retrofit of an existing tool —
  `delete_assignment` **does not exist anywhere in the codebase** (grepped `src/`, zero hits).
  It's a genuinely new destructive capability, the one piece of this request that isn't "add a
  safety wrapper to something already shipping." Worth a deliberate yes on its own, plus the
  #204 tool-annotation treatment (`destructiveHint`/`idempotentHint`) that every other write
  tool gets.
  ...
  The good news: this isn't a new mechanism to design, and it's more mechanical than I first
  said — there's one shared ConfirmationGuard class (core/write_confirmation.py) already used by
  content_migrations.py and four times in messaging.py. This is import-and-instantiate for six
  of the seven tools, not independent implementations to harmonize.

  The seventh is different, though: delete_assignment_with_confirmation isn't a safety wrapper
  on something that already exists — there's no delete_assignment tool at all today. So that
  one's a real "should this exist" decision, not just a mechanical retrofit, and I want your
  explicit yes on it before I build it alongside the other six.
  ...
  Planning to do the six retrofits as one PR given how mechanical the pattern reuse is, and treat
  the new assignment-delete tool as a separate decision — let me know if you'd rather see it
  split up.
  ```
