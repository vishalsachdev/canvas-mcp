...
A self-hosted [Model Context Protocol](https://modelcontextprotocol.io) (MCP) server that lets an LLM client automate Canvas LMS work on behalf of an instructor, course designer, or TA. The **primary value is course operations at scale** — building, modifying, QA/accessibility-checking, and copying course structures, pages, modules, and assignments across one or many courses. It also supports student-facing workflows (grading, discussion facilitation, peer-review analytics, messaging). Notably, the highest-volume course-build/QA/copy work operates on **course content, not identifiable student records**, so much of it carries a lower data-sensitivity profile than the grading/analytics workflows. Architecture as deployed/intended:
