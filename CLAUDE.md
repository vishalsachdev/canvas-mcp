...
- [~] Issue #115: Gies/Azure hosted deployment — v1 (key gate) shipped (#121); **v2 Entra platform-auth cut over to staging 2026-06-14** (App Service Easy Auth API mode + PRM; per-request `X-MS-CLIENT-PRINCIPAL` header-reader, PR #125). **Blocker found:** mcp-remote/Claude-Desktop can't do the Entra token exchange (`AADSTS9010010`; VS Code works). **Fix in progress:** verified custom domain `canvas-mcp.disruptionlab.illinois.edu` (App ID URI registered; DNS request pending → cloud routine watching). Remaining: send DNS email; bind hostname+cert; AcrPull RBAC fix (needs Adam)
...
- **Auth model (v2 — Entra platform auth, LIVE 2026-06-14; supersedes v1 access-key).** Azure App Service authentication fronts the endpoint in **API/bearer mode** (`authsettingsV2`: `requireAuthentication=true`, `unauthenticatedClientAction=Return401`, token store on) and validates the Entra token itself; `WEBSITE_AUTH_PRM_DEFAULT_WITH_SCOPES` makes it serve RFC 9728 PRM + a `401 WWW-Authenticate` challenge (NOT a 302, so non-browser MCP clients work). The app then reads the trusted `X-MS-CLIENT-PRINCIPAL-ID` (`ENTRA_AUTH_ENABLED=true`, paired with `MCP_ALLOW_UNAUTHENTICATED=true`), authorizes against `MCP_ENTRA_ALLOWED_OIDS` (empty = any tenant user via the allowed client app), audit-logs the oid. URL still server-pinned; per-user `X-Canvas-Token` unchanged; `X-Canvas-URL` ignored. Validated: no-token → 401 + challenge with our scope; PRM doc served; user sign-in (NetID+Duo+consent) succeeds.
  - ⚠️ **`mcp-remote` (and Claude Desktop's native connector) CANNOT complete the Entra token exchange** — `AADSTS9010010` "resource doesn't match scopes." Root cause (source-confirmed): the MCP SDK sends the PRM `resource` (the app URL) to Entra's token endpoint, but Entra rejects it because the app URL isn't a registered App ID URI (`api://…` is). Known/open: `geelen/mcp-remote#217`, `anthropics/claude-code#52871`. **VS Code's native MCP client works** (Microsoft's documented path). So today the Entra endpoint is only reachable by VS Code; mcp-remote/Claude-Desktop users need the fix below.
  - **Fix in progress (no proxy): verified custom domain.** Entra won't accept `*.azurewebsites.net` as an App ID URI (unverified domain), but it **accepts `illinois.edu` subdomains**. Registered `https://canvas-mcp.disruptionlab.illinois.edu/mcp` as an App ID URI on the API app (done). Once the app is served at that hostname, the PRM `resource` == the registered identifier → `mcp-remote`'s resource matches the scope **natively, all clients work**. Pending: DNS record (`docs/tech-services-dns-request.txt` → `consult@illinois.edu`; UniQuick precedent = `quick.disruptionlab.illinois.edu`, ITP actions silently/fast), then bind hostname + managed cert + repoint PRM scope/`allowedAudiences` to the hostname. A cloud routine (claude.ai/code/routines `trig_01LpyxTPtFpp29B5jzDsiS1R`, daily noon CDT) polls DNS and pings when it lands.
...
- **Client setup (v2 — Entra OAuth, any MCP client via `mcp-remote`):** point `mcp-remote` at `…/mcp` with the pre-authorized client + named scope, and pass each user's own Canvas token as a header. On first connect it opens the browser for Illinois SSO/Duo, then sends the Entra bearer per request:
  ```
  "args": ["-y","mcp-remote","https://gies-canvas-mcp-staging.azurewebsites.net/mcp",
  ...
  ```
  Verified working with `mcp-remote@0.1.38` (writes the scope verbatim; static client → no dynamic registration). If `api://…` scope ever trips `AADSTS90009`, fall back to the bare-GUID form `e1443fda-…/access_as_user`. Works the same across Claude Desktop/Code, Codex, VS Code, Cursor.
