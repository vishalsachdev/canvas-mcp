...
- **Validated + onboarded (6/14):** TA **Lalitha** connected via Claude Desktop + `mcp-remote` and confirmed it works — first real positive end-to-end test. Per-user keys minted for **Lalitha, Ash, Cheng (L&D)** (gate now 4 keys; holders mapped in Hosted Deployment above); per-person onboarding `.txt` files on `~/Desktop`. Corrected the AcrPull authority: it needs a **subscription Owner = Adam King** (not Ash — verified via RBAC the whole app team is Contributor-only); Adam request drafted at `~/Desktop/acrpull-grant-request.txt`. Recorded the RBAC gotcha in `agent-infra/azure-infra-analysis-2026-06.md`.
- Next: (1) send Adam the `AcrPull` grant → then retire the ACR admin-user workaround (re-enable MI pull, disable admin user); (2) decide staging-vs-prod app for the SBC 529 term (Jul 6–Aug 16); (3) Entra/OAuth v2; (4) standing queue: backlog triage + #106 mypy cleanup.
