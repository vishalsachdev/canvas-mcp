...
- [x] Issue #115: Gies/Azure hosted deployment — **DONE 2026-06-17.** v2 Entra platform-auth (#125) + custom domain `canvas-mcp.disruptionlab.illinois.edu` (bound + managed cert) **resolves the `AADSTS9010010` mcp-remote blocker — verified live, all clients work.** App renamed `gies-canvas-mcp` → `canvas-mcp` (house-consistent; old apps deleted). Branch→slot CI added (#128/#129). Remaining polish: tighten `MCP_ENTRA_ALLOWED_OIDS`; AcrPull RBAC fix (needs Adam, still on ACR admin-user creds)
...
- **Endpoint**: `https://canvas-mcp.disruptionlab.illinois.edu/mcp` (streamable-http; custom domain bound + GeoTrust managed cert, SNI). Subscription `urbana-business-disruptionlab`, RG `DL_ResourceGroup_01`, app **`canvas-mcp`** (prod, default Production slot + a `staging` slot) on shared plan `dl-appplan-01`, registry `giescanvasmcpacr`. The app's default host `canvas-mcp.azurewebsites.net` is the CNAME target (backend only). **App renamed 2026-06-17** from `gies-canvas-mcp` → `canvas-mcp` (house-consistent bare name, matches mindforum/uniquick/illinihunt); the old `gies-canvas-mcp` + `gies-canvas-mcp-staging` apps were deleted.
- **Auth model (v2 — Entra platform auth, LIVE 2026-06-14; supersedes v1 access-key).** Azure App Service authentication fronts the endpoint in **API/bearer mode** (`authsettingsV2`: `requireAuthentication=true`, `unauthenticatedClientAction=Return401`, token store on) and validates the Entra token itself; `WEBSITE_AUTH_PRM_DEFAULT_WITH_SCOPES` makes it serve RFC 9728 PRM + a `401 WWW-Authenticate` challenge (NOT a 302, so non-browser MCP clients work). The app then reads the trusted `X-MS-CLIENT-PRINCIPAL-ID` (`ENTRA_AUTH_ENABLED=true`, paired with `MCP_ALLOW_UNAUTHENTICATED=true`), authorizes against `MCP_ENTRA_ALLOWED_OIDS` (empty = any tenant user via the allowed client app), audit-logs the oid. URL still server-pinned; per-user `X-Canvas-Token` unchanged; `X-Canvas-URL` ignored. Validated: no-token → 401 + challenge with our scope; PRM doc served; user sign-in (NetID+Duo+consent) succeeds.
  - ✅ **mcp-remote blocker RESOLVED 2026-06-17 via the custom domain (no proxy).** Previously `mcp-remote`/Claude-Desktop failed with `AADSTS9010010` "resource doesn't match scopes" — the MCP SDK sends the PRM `resource` (the app URL) to Entra's token endpoint, and Entra rejected it because `*.azurewebsites.net` isn't a registered App ID URI. Fix: registered `https://canvas-mcp.disruptionlab.illinois.edu/mcp` as an App ID URI on the API app, bound that hostname to the app, so the PRM `resource` served there == the registered identifier. Also added that URI to Easy Auth `allowedAudiences` (the RFC 8707 token `aud` is the resource). **Verified live**: `mcp-remote` completes the token exchange + establishes the MCP session. All clients (Claude Desktop/Code, Cursor, Codex, VS Code) now work. (Refs for the underlying SDK bug: `geelen/mcp-remote#217`, `anthropics/claude-code#52871`.)
  - **Entra app registrations (Illinois tenant `44467e6f-462c-4ea2-823f-7800de5434e3`):** API = `Canvas MCP API` appId `e1443fda-5aa7-4136-a884-d97f64258ef0` (scope `api://e1443fda-…/access_as_user`); client = `Canvas MCP Desktop Client` appId `f0a9c95b-a9f3-4918-94ae-2818615bf085` (public, redirect `http://localhost:3334/oauth/callback`), **pre-authorized** on the API scope (no admin consent). App Service `allowedApplications` = the client appId. IDs are not secrets (queryable via `az ad app list`).
- **Auth model (v1 — access-key, SUPERSEDED by v2):** clients sent `X-MCP-Access-Key` (gate; `MCP_ACCESS_KEYS`) + their own `X-Canvas-Token`. Still the code path when `ENTRA_AUTH_ENABLED` is unset (e.g. local/stdio). `CANVAS_API_TOKEN` must never be set in HTTP mode (startup guard exits). Code-exec off; `httpsOnly=true`.
- **Deploy (CI, 2026-06-17): branch→slot GitHub Actions.** Push to `main` → `.github/workflows/deploy-prod.yml` builds the image, pushes to ACR, deploys to the **Production** slot (auto, no gate). Push to a `staging` branch → `deploy-staging.yml` → the **staging** slot. Auth = ACR admin creds + per-slot publish profiles (GitHub secrets `ACR_USERNAME`/`ACR_PASSWORD`/`AZURE_WEBAPP_PUBLISH_PROFILE_PROD`/`_STAGING`) — no service principal (sidesteps the Owner-only RBAC wall). ⚠️ SCM basic-auth publishing must stay ENABLED on the app + slot or `webapps-deploy` fails with "Failed to get app runtime OS". Pattern recorded in the `illinois-azure-container-deploy` skill §3c. (Manual fallback: `az acr build … -t canvas-mcp:azure-gies-$(git rev-parse --short HEAD) .` → `az webapp config container set …`.)
...
- **Client setup (v2 — Entra OAuth, any MCP client via `mcp-remote`):** point `mcp-remote` at `…/mcp` with the pre-authorized client + named scope, and pass each user's own Canvas token as a header. On first connect it opens the browser for Illinois SSO/Duo, then sends the Entra bearer per request:
  ```
  "args": ["-y","mcp-remote","https://canvas-mcp.disruptionlab.illinois.edu/mcp",
           "--header","X-Canvas-Token:<YOUR_CANVAS_TOKEN>",
           "--static-oauth-client-info","{\"client_id\":\"f0a9c95b-a9f3-4918-94ae-2818615bf085\"}",
           "--static-oauth-client-metadata","{\"scope\":\"api://e1443fda-5aa7-4136-a884-d97f64258ef0/access_as_user\"}"]
  ```
  **Verified end-to-end 2026-06-17** against the custom domain (mcp-remote completes the Entra exchange + MCP session). Static client → no dynamic registration. If `api://…` scope ever trips `AADSTS90009`, fall back to the bare-GUID form `e1443fda-…/access_as_user`. Works the same across Claude Desktop/Code, Codex, VS Code, Cursor.

## Session Log
> Full history: [docs/session-history.md](./docs/session-history.md)

### 2026-06-17 — mcp-remote blocker RESOLVED + app→`canvas-mcp` + branch→slot CI
- **🏁 The `AADSTS9010010` blocker is gone — verified live.** DNS landed (CNAME + asuid), bound `canvas-mcp.disruptionlab.illinois.edu` + GeoTrust managed cert to the app; PRM `resource` now == the registered App ID URI, and added that URI to Easy Auth `allowedAudiences` (RFC 8707 token `aud`). Ran `mcp-remote` end-to-end against the custom domain: token exchange + MCP session succeed. **All clients (Claude Desktop/Code, Cursor, Codex, VS Code) work.**
...
- **Cut staging over to Entra (live, `az`-driven):** created 2 Entra app regs (API `e1443fda…` + pre-authorized public client `f0a9c95b…`), enabled Easy Auth API mode (`Return401` + PRM), validated 401+RFC9728 challenge on the wire, container healthy.
...
- **Pivot (no proxy): verified custom domain.** Entra rejects `*.azurewebsites.net` App ID URIs but **accepts `illinois.edu` subdomains** — registered `https://canvas-mcp.disruptionlab.illinois.edu/mcp` (tested live). Confirmed via sibling repo that `disruptionlab.illinois.edu` is a live IPAM zone and UniQuick already runs at `quick.disruptionlab.illinois.edu` (ITP actions DNS requests silently/fast). DNS request drafted (`docs/tech-services-dns-request.txt`); **cloud routine** `trig_01LpyxTPtFpp29B5jzDsiS1R` polls DNS daily at noon CDT and pings when it lands.
